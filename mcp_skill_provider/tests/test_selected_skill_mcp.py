#!/usr/bin/python
# -*- coding: utf-8 -*-
"""Live smoke test for a selected Harness skill via mcp_skill_provider.
...
selected skill metadata, and executes a selected argv through MCP tools/call.

Example:
    python mcp_skill_provider/tests/test_selected_skill_mcp.py \
...
def _mcp_call(
    ...
    data = _request_json(
        "POST",
        f"{base_url.rstrip('/')}/{endpoint_id}/mcp",
        headers={"Authorization": f"Bearer {token}", "Part-Id": part_id},
        json_body={
            "jsonrpc": "2.0",
            "id": request_id,
...
def _extract_argv_tokens(raw_args: List[str]) -> tuple[List[str], Optional[List[str]]]:
    ...
    known_flags = {"--reload-mcp", "--expect-stdout", "--summary-only", "--print-stdout"}
    known_options = {
        "--base-url",
        "--endpoint-id",
        "--part-id",
