...
> **Date:** 2026-08-25
> **Status:** v0.0.1 package scaffold complete; runtime registration with `mcp_daemon_engine` wired (P1.5 done) — integration testing (P2) not yet started
...
| `get_skill(name)` | Retrieve full skill instructions and metadata (triggers on-demand S3 refresh) |
...
Verified against the sibling `mcp_daemon_engine` and `harness_engineering_engine` repos on 2026-08-25:
...
harness_engineering_engine
  GraphQL endpoint → services → DynamoDB / S3 / local skill cache
```
